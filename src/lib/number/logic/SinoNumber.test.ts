...
describe('sinoCardinal/Counter fromNumber', () => {
  ...
  it('throws errors for numbers outside of [0, 10^15]', () => {
    ...
    expect(sinoCounter.fromNumber(okay).hangul).toBe('구백구십구조 구천구백구십구억 구천구백구십구만 구천구백구십구');
  });

  it('does not show any unit if any units place digit is 1', () => {
    expect(sinoCardinal.fromNumber(111_1111_1111_1111).hangul).toBe('백십일조 천백십일억 천백십일만 천백십일');
  });
