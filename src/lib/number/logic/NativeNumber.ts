import { HanNumber, type NativeNumberOption, type HangulNumberObj } from './HanNumber';
...
export class NativeNumber extends HanNumber {
  ...
  fromNumber = (number: number): HangulNumberObj => {
    ...
    else hangul = [NativeNumber.nativeMap['native'].tens[tensDig], NativeNumber.nativeMap[type].ones[onesDig]].join('');
