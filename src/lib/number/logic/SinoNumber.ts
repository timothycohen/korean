import { HanNumber, type SinoNumberOption, type HangulNumberObj } from './HanNumber';
...
export class SinoNumber extends HanNumber {
  ...
  fromNumber = (number: number): HangulNumberObj => {
    ...
    const calcUnitOfFour = (index: number): string => {
      ...
      if (orderOfMagnitude > maxOOM)
        throw new Error(`largest unit is ${SinoNumber.sinoMap[maxOOM]} with an order of magnitude of ${maxOOM}`);
  ...
  isValid = (numAsString: string, option: 'possible' | 'abs' | 'local' = 'possible'): boolean => {
    ...
    if (option === 'local' && (num > this.OOMToMax(this._max) || num < this.OOMToMin(this._min))) return false;
    // only allow when number is within absMin/absMax
    if (option === 'abs' && (num > this.OOMToMax(this.absMax) || num < 10 ** this.OOMToMin(this.absMin))) return false;
