...
export const GET: RequestHandler = async ({ url }) => {
  ...
  const user = await auth.getUser('oauth:github', email);
  if (user) {
    try {
      const authenticateUser = await auth.authenticateUser('oauth:github', email);
      return {
        status: 302,
        headers: {
  ...
  try {
    const createUser = await auth.createUser('oauth:github', email, {
      user_data: {
        email,
      },
    });
