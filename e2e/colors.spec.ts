...
test.describe('colors', () => {
  test.describe.parallel('general', () => {
    ...
    test('load hangul -> color page by default', async ({ page }) => {
      ...
      await expect(page.locator('div.selection-container')).toBeVisible();
    });
    ...
    test('clicking the direction button changes views', async ({ page }) => {
      ...
      await page.waitForSelector('#korean-color', { timeout: 2000 });
    });
  ...
  test.describe.parallel('hangul to color', () => {
    ...
    test('clicking the answer button makes the color of the 한글 and the page background the goal color', async ({
      ...
      await page.goto('/colors');
      const goalEl = page.locator('.korean-container h1');
    ...
    test('clicking the wrong answer plays a fail animation on that button', async ({ page }) => {
      await page.goto('/colors');
      const goalText = await page.textContent('.korean-container h1');
    ...
    test('clicking the correct answer changes the color (no duplicates) and shuffles', async ({ page }) => {
      await page.goto('/colors');
      let goalText = await page.textContent('.korean-container h1');
      const order = await page.textContent('.selection-container');
      ...
      expect(await page.textContent('.korean-container h1')).not.toBe(goalText);
      expect(await page.textContent('.selection-container')).not.toEqual(order);
    });
  ...
  test.describe.parallel('color to hangul', () => {
    ...
    test('clicking the key toggle shows key', async ({ page }) => {
      ...
      expect(await page.locator('.key-container').count()).toBeFalsy();
      ...
      await page.waitForSelector('.key-container');
    });

    test('clicking answer toggle shows answer', async ({ page }) => {
      await setup(page);

      expect(await page.locator('.korean-container h1').count()).toBeFalsy();
      ...
      await page.waitForSelector('.korean-container h1');
    });
    ...
    test('typing the correct answer changes the goal', async ({ page }) => {
      ...
      const goalText1 = await page.textContent('.korean-container');
      const input = page.locator('#korean-color');
      await input.type(goalText1 ?? '');

      const goalText2 = await page.textContent('.korean-container');
    ...
    test('no phantom input', async ({ page }) => {
      ...
      for (let i = 0; i < 5; i++) {
        await input.type((await page.textContent('.korean-container'))?.trim() ?? '');
